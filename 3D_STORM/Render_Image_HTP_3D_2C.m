...
savename   = 'Tom20_A647_10ms_3';
savepath   = 'Z:\Christian-Sieben\data_HTP\2017-05-22_3D_Test_Mito\locResults\Tom20_A647_10ms_3';

locpath_Ch1     = 'Z:\Christian-Sieben\data_HTP\2017-05-19_3D_Test_Centriole\locResults_Cent\A647_COT_500mW_10ms_1';
locname_Ch1     = 'A647_COT_500mW_10ms_1_MMStack_1_Localizations_DC_Z';

locpath_Ch2     = 'Z:\Christian-Sieben\data_HTP\2017-05-19_3D_Test_Centriole\locResults_Cent\DL755_COT_500mW_20ms_1';
locname_Ch2     = 'DL755_COT_500mW_20ms_1_MMStack_1_Localizations_DC_corrected_Z';
...
cd(locpath_Ch2);
locs_Ch2        = dlmread([locname_Ch2 '.csv'],',',1,0);
...
filter3 = find(locs_Ch2(:,zCol)==0);
locs_Ch2(filter2,:) = [];

figure('Position',[700 600 800 500]); 
subplot(2,3,1);
hist(locs_Ch2(locs_Ch2(:,photonsCol)<2e4,photonsCol),50);
title('Photons');
subplot(2,3,2);
hist(locs_Ch2(:,sigmaXCol),50);
title('Sigma X');
subplot(2,3,3);
hist(locs_Ch2(locs_Ch2(:,LLCol)<1500,LLCol),50);
title('LL');
subplot(2,3,4);
hist(locs_Ch2(locs_Ch2(:,uncertaintyCol)<100,uncertaintyCol),50);
title('Uncertainty');
subplot(2,3,5);
hist(locs_Ch2(:,sigmaYCol),50);
title('Sigma Y');
subplot(2,3,6);
hist(locs_Ch2(:,zCol),50);
title('Z Range');
...
% Set Filter parameters for Channel 2
...
filter   = find(locs_Ch2(:,zCol) < MaxZ & locs_Ch2(:,zCol) > MinZ & locs_Ch2(:,photonsCol) > MinPhotons & locs_Ch2(:,uncertaintyCol) < Maxuncertainty & locs_Ch2(:,framesCol) > minFrame & locs_Ch2(:,LLCol) > logLikelihood);
locs_Filtered_Ch2 = locs_Ch2(filter,1:end);

fprintf('\n -- Ch2 Filtered %f of the locs are left --\n', ((length(locs_Filtered_Ch2)/length(locs_Ch2))));
...
vx      = find(locs_Filtered_Ch2(:,xCol)>xmin & locs_Filtered_Ch2(:,xCol)<xmax & locs_Filtered_Ch2(:,yCol)>ymin & locs_Filtered_Ch2(:,yCol)<ymax);
subset2 = locs_Filtered_Ch2(vx,1:end);
...
figure('Position',[1200 400 500 500])
scatter(subset2(:,xCol),subset2(:,yCol),2,subset2(:,zCol))
colormap('jet');
colorbar
...
colorbar
scatter3(subset2(:,xCol),subset2(:,yCol),subset2(:,zCol),2,'filled');hold on;

locs_Filtered_Ch1 = subset2;
